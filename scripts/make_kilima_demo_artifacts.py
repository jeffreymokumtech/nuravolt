...
# ---------------------------------------------------------------------------
# Synthetic-donor mode (--synthetic-donor)
#
# Everything below generates the same artifacts from a rain-reset soiling
# model instead of the anonymised donor plant, so the demo carries no series
# derived from measured data. Precipitation comes from the committed
# rain_history.json (public Open-Meteo archive for the fictional site's
# coordinates); rates and noise are deterministic per inverter.
# ---------------------------------------------------------------------------

SYNTH_NOTE = (
    "Synthetic demo series generated by scripts/make_kilima_demo_artifacts.py "
    "--synthetic-donor: a rain-reset soiling model over public Open-Meteo "
    "precipitation for this fictional site. Not measurements."
)
SYNTH_DAYS = 365
BASE_RATE = 0.0012          # SR lost per dry day, fleet median
DIRTY_FACTOR = 2.5          # INV-03 fouls faster (matches seed_e2e_demo_data)
FLOOR_SR = 0.80


def _jit(key: str, lo: float, hi: float) -> float:
    h = int(hashlib.md5(key.encode()).hexdigest(), 16) % 10_000
    return lo + (hi - lo) * h / 9_999.0


def _rain_by_date() -> dict[str, float]:
    rain = json.load(open(OUT / "rain_history.json"))
    return {r["date"]: float(r.get("precipitation_mm") or 0.0) for r in rain.get("daily_data", [])}


def _rain_on(rain: dict[str, float], d: date) -> float:
    """Precipitation for a date; when the committed history does not cover it,
    reuse the same calendar day a year earlier (climatology stand-in)."""
    for back in (0, 1, 2):
        v = rain.get(date(d.year - back, d.month, min(d.day, 28)).isoformat())
        if v is not None:
            return v
    return 0.0


def _simulate(inv_id: str, days: int, end: date, rain: dict[str, float], rate: float) -> list[tuple[date, float]]:
    sr = 0.985
    out = []
    for i in range(days, -1, -1):
        ...
        mm = _rain_on(rain, d)
        if mm >= HEAVY_RAIN_MM:
            sr = min(1.0, sr + 0.85 * (1.0 - sr))
        elif mm >= RECOVERY_MM:
            sr = min(1.0, sr + 0.45 * (1.0 - sr))
        else:
            sr = max(FLOOR_SR, sr - rate * (0.8 + 0.4 * _jit(f"{inv_id}:{d}", 0, 1)))
        noise = (_jit(f"{inv_id}:n:{d}", -1, 1)) * 0.003
        out.append((d, round(min(1.0, max(FLOOR_SR, sr + noise)), 4)))
    return out


def build_per_inverter_synthetic() -> None:
    rain = _rain_by_date()
    end = date.today() - timedelta(days=1)
    ids = [f"INV-{i + 1:02d}" for i in range(N_TARGET)]
    inverters = {}
    for inv_id in ids:
        rate = BASE_RATE * _jit(f"{inv_id}:rate", 0.75, 1.25) * (DIRTY_FACTOR if inv_id == DIRTY_TARGET else 1.0)
        series = _simulate(inv_id, SYNTH_DAYS, end, rain, rate)
        history = [{"date": d.isoformat(), "sr": v, "confidence": 0.85} for d, v in reversed(series)]
        last7 = [v for _, v in series[-7:]]
        inverters[inv_id] = {
            "current_sr": series[-1][1],
            "sr_7d_avg": round(sum(last7) / len(last7), 4),
            "confidence": 0.85,
            "history": history,
            "n_source_series": 1,
        }
    # Calibrate to the seeded DB fleet mean so the static specimen and the
    # DB-backed KPI on the soiling page agree.
    target = db_fleet_sr_mean()
    fleet_mean = sum(r["current_sr"] for r in inverters.values()) / len(inverters)
    if target and fleet_mean:
        f = target / fleet_mean
        print(f"  calibrating synthetic SR x{f:.4f} (fleet {fleet_mean:.4f} -> {target:.4f})")
        for rec in inverters.values():
            rec["current_sr"] = round(min(1.0, rec["current_sr"] * f), 4)
            rec["sr_7d_avg"] = round(min(1.0, rec["sr_7d_avg"] * f), 4)
            for h in rec["history"]:
                h["sr"] = round(min(1.0, h["sr"] * f), 4)
    meta = {
        "generated_at": datetime.now().isoformat(),
        "model_variant": "synthetic_rain_reset_v1",
        "n_inverters": N_TARGET,
        "plant_id": SLUG,
        "provenance": {
            "source": "synthetic",
            "method": "rain_reset_soiling_model",
            "data_start": (end - timedelta(days=SYNTH_DAYS)).isoformat(),
            "data_end": end.isoformat(),
            "stats_window": {"start": (end - timedelta(days=90)).isoformat(), "end": end.isoformat()},
            "note": SYNTH_NOTE,
        },
    }
    dump(f"per_inverter/{SLUG}_per_inverter_sr.json", {"metadata": meta, "inverters": inverters})
...
def build_cleaning_synthetic() -> None:
    per_inv = json.load(open(OUT / "per_inverter" / f"{SLUG}_per_inverter_sr.json"))
    fleet_sr = sum(r["current_sr"] for r in per_inv["inverters"].values()) / len(per_inv["inverters"])
    annual_mwh = 6.0 * 8760 * 0.215
    baseline_revenue = round(annual_mwh * PRICE_EUR_MWH, 2)
    n = len(CLEANING_DATES)
    energy = TARGET_ENERGY_MWH
    revenue = round(energy * PRICE_EUR_MWH, 2)
    cost = COST_PER_CLEANING_EUR * n
    ...
    roi = round(net / cost * 100, 1)
    proposal = {
        "metadata": {"plant_id": SLUG, "generated_at": datetime.now().isoformat(), "note": SYNTH_NOTE + " Costs are estimates; update with actual values."},
        "executive_summary": {"plant_capacity_MW": 6.0, "forecast_period": FORECAST_PERIOD, "recommended_cleanings": n, "expected_net_benefit_EUR": net, "expected_roi_pct": roi},
        "optimal_schedule": {"cleaning_dates": CLEANING_DATES, "n_cleanings": n, "energy_recovered_MWh": energy, "revenue_recovered_EUR": revenue, "cleaning_cost_EUR": cost, "net_benefit_EUR": net, "roi_pct": roi, "avg_sr": round(min(0.99, fleet_sr + 0.03), 3)},
        "financial_analysis": {"baseline_revenue_EUR": baseline_revenue, "optimized_revenue_EUR": revenue, "cleaning_investment_EUR": cost, "net_benefit_EUR": net, "payback_days": round(cost / (revenue / 365.0), 1)},
        "technical_details": {
            "avg_soiling_ratio_baseline": round(fleet_sr, 4),
            "avg_soiling_ratio_optimized": round(min(0.99, fleet_sr + 0.03), 4),
            "energy_recovered_MWh": energy,
            "avg_soiling_loss_baseline_pct": round((1 - fleet_sr) * 100, 2),
            "avg_soiling_loss_optimized_pct": round((1 - min(0.99, fleet_sr + 0.03)) * 100, 2),
        },
        "recommendations": [
            "Clean in the dry seasons (January to February, June to September); the long rains in March to May and the short rains in October to December recover most of the deposit for free.",
            f"INV-03 fouls about {DIRTY_FACTOR:.1f}x faster than the fleet median; inspect its row for a local dust source before the next fleet cleaning.",
        ],
    }
    dump("operator_proposal.json", proposal)
    scenarios = []
    ladder = {1: 0.72, 2: 0.88, 3: 1.0, 4: 1.06}
    dates_by_n = {1: ["2026-08-25"], 2: ["2026-08-18", "2027-01-26"], 3: CLEANING_DATES, 4: ["2026-08-04", "2026-09-15", "2027-01-12", "2027-02-16"]}
    for n_c, frac in ladder.items():
        e = round(TARGET_ENERGY_MWH * frac, 1)
        rev = round(e * PRICE_EUR_MWH, 2)
        c = COST_PER_CLEANING_EUR * n_c
        scenarios.append({"strategy": f"{n_c} cleaning(s)", "dates": ", ".join(dates_by_n[n_c]), "energy_recovered_mwh": e, "revenue_recovered_eur": rev, "cleaning_cost_eur": c, "net_benefit_eur": round(rev - c, 2), "roi_pct": round((rev - c) / c * 100, 1), "avg_sr": round(min(0.99, fleet_sr + 0.01 * n_c), 3), "roi__pct": round((rev - c) / c * 100, 1)})
    dump("cleaning_comparison.json", {"metadata": {"plant_id": SLUG, "generated_at": datetime.now().isoformat(), "note": SYNTH_NOTE}, "scenarios": scenarios})


def build_ml_forecast_synthetic() -> None:
    rain = _rain_by_date()
    per_inv = json.load(open(OUT / "per_inverter" / f"{SLUG}_per_inverter_sr.json"))
    fleet_sr = sum(r["current_sr"] for r in per_inv["inverters"].values()) / len(per_inv["inverters"])
    start = date.today()
    series = _simulate("fleet", SYNTH_DAYS - 1, start + timedelta(days=SYNTH_DAYS - 1), rain, BASE_RATE)
    # Anchor the forward curve on today's fleet SR.
    offset = fleet_sr - series[0][1]
    forecasts = []
    for i, (d, v) in enumerate(series):
        sr = round(min(1.0, max(FLOOR_SR, v + offset * max(0.0, 1 - i / 60.0))), 4)
        band = round(0.01 + 0.03 * min(1.0, i / 120.0), 4)
        forecasts.append({"date": d.isoformat(), "sr_physics": 0.0, "sr_ml_correction": 0.0, "sr_predicted": sr, "soiling_loss_pct": round((1 - sr) * 100, 3), "is_cleaning_needed": sr < 0.95, "sr_lower_bound": round(max(FLOOR_SR, sr - band), 4), "sr_upper_bound": round(min(1.0, sr + band), 4)})
    dump("ml_forecast_365d.json", {"metadata": {"plant_id": SLUG, "forecast_period": {"start": forecasts[0]["date"], "end": forecasts[-1]["date"]}, "n_days": len(forecasts), "model_type": "synthetic_rain_reset", "model_version": "synthetic_rain_reset_v1", "climate_zone": "EQUATORIAL_EAST_AFRICA", "dominant_dust_source": "local agricultural + road dust", "generated_at": datetime.now().isoformat(), "note": SYNTH_NOTE}, "forecasts": forecasts})


def main_synthetic() -> int:
    print("[kilima] synthetic-donor mode")
    OUT.mkdir(parents=True, exist_ok=True)
    if not (OUT / "rain_history.json").exists():
        build_rain_history()          # public Open-Meteo archive, one-off network fetch
    if not (OUT / "aod_history.json").exists():
        build_aod_history()
    build_per_inverter_synthetic()
    build_cleaning_synthetic()
    build_ml_forecast_synthetic()
    build_dustiq_history()
    build_loss_waterfall()
    return 0
...
if __name__ == "__main__":
    if "--synthetic-donor" in sys.argv:
        raise SystemExit(main_synthetic())
